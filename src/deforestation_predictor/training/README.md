...
| **`train_experiment.py`** | The main entry point for training DL models (e.g., `3dcnn`, `resunet`). Handles the training loop, validation, and checkpointing. |
...
[cite_start]We use **Focal Loss** instead of standard Cross Entropy because deforestation is a rare event (class imbalance)[cite: 31].
...
* **`--model_type`**: The architecture to use.
    * Options: `3dcnn` (Baseline RQ1) or `resunet` (if implemented).
* **`--context_months`**: (RQ2) Number of past months to use as input.
    * Default is `12`.
...
* The best model (highest Val PR-AUC) is saved to `checkpoints/best_model.pth`.
...
```bash
# Note: Check the 'CONFIG' section at the top of the file to set your DATA_ROOT before running.
