...
def main(parser):
    ...
    for tile in args.tiles:
        ...
        if os.path.isfile(os.path.join(args.dump_path, '{}_gt_tr.npy'.format(tile))):
            ...
            dynam_var_train, gt_stack_tr, dynam_var_val, gt_stack_val, dynam_var_test, gt_stack_test, mask = prepare_data(
                tile, logger)
...
def prepare_data(tile, logger):
    ...
    for var, max_val in args.dynamic_var:
        ...
        logger.info(
            f'{var} stack shape {train_stack.shape} with max val {np.max(train_stack[~np.isnan(train_stack)])} and min val {np.min(train_stack[~np.isnan(train_stack)])}')
    ...
    for var, max_val in args.auto_gen_dynamic:
        ...
        if var == 'month':
            train_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * int(x.split('_')[-2].split('-')[1]) for x in
                 train_split], axis=0).astype('float16') / maximum
            val_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * int(x.split('_')[-2].split('-')[1]) for x in
                 val_split], axis=0).astype('float16') / maximum
            test_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * int(x.split('_')[-2].split('-')[1]) for x in
                 test_split], axis=0).astype('float16') / maximum

        elif var == 'sinmonth':
            train_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * math.sin(int(x.split('_')[-2].split('-')[1]))
                 for x in train_split], axis=0).astype('float16') / maximum
            val_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * math.sin(int(x.split('_')[-2].split('-')[1]))
                 for x in val_split], axis=0).astype('float16') / maximum
            test_stack = np.stack(
                [np.ones((train_stack.shape[1], train_stack.shape[2])) * math.sin(int(x.split('_')[-2].split('-')[1]))
                 for x in test_split], axis=0).astype('float16') / maximum

        elif var == 'monthssince2019':
            train_stack = np.stack([np.ones((train_stack.shape[1], train_stack.shape[2])) * months_since_january_2019(
                Path(x).stem.split('_')[2][2:-3]) for x in train_split], axis=0).astype('float16') / maximum
            val_stack = np.stack([np.ones((train_stack.shape[1], train_stack.shape[2])) * months_since_january_2019(
                Path(x).stem.split('_')[2][2:-3]) for x in val_split], axis=0).astype('float16') / maximum
            test_stack = np.stack([np.ones((train_stack.shape[1], train_stack.shape[2])) * months_since_january_2019(
                Path(x).stem.split('_')[2][2:-3]) for x in test_split], axis=0).astype('float16') / maximum
    ...
    for var, max_val in args.yearly_var:
        ...
        logger.info(
            f'{var} stack shape {yearly_var.shape} with max val {np.max(yearly_var[~np.isnan(yearly_var)])} and min val {np.min(yearly_var[~np.isnan(yearly_var)])}')
    ...
    for var, max_val in args.yearly_var:
        ...
        for yr in list_year_test:
            try:
                ...
                img = read_tiff(glob.glob(data_input + '/*{}*{}*'.format(int(yr) - 1, var) + '*.tif')[0]).astype(
                    'float32')
    ...
    if len(args.static_names) > 0:
        ...
        for var, max_val in args.static_names:
            if var == 'xy':
                ...
                logger.info(
                    f'{var} stack shape {img.shape} with max val {np.max(img[~np.isnan(img)])} and min val {np.min(img[~np.isnan(img)])}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_input', type=str,
                        default='C:/PostDoc/deforestation_project/data/tiles_march_2025/input',
